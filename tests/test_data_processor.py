...
class TestAttendanceProcessor:
    ...
    def test_process_checkins_to_dataframe_multiple_days(self):
        ...
        checkin_data = [
            ...
            },
            {
                "employee": "EMP001",
                "employee_name": "John Doe",
                "time": "2025-01-02T08:45:00",
            },
            {
                "employee": "EMP001",
                "employee_name": "John Doe",
                "time": "2025-01-02T17:15:00",
            },
        ]
    ...
    def test_calcular_horas_descanso_insufficient_checkins(self):
        ...
        test_row = pd.Series(
            {"checado_1": "08:00:00", "checado_2": "17:00:00", "checado_3": None}
        )
    ...
    def test_procesar_horarios_con_medianoche_no_midnight_crossing(
        self, mock_obtener_horario
    ):
    ...
    @patch("data_processor.obtener_horario_empleado")
    def test_procesar_horarios_con_medianoche_with_midnight_crossing(
        self, mock_obtener_horario
    ):
    ...
    def test_aplicar_calculo_horas_descanso_with_breaks(self):
        ...
        assert (
            result.iloc[0]["horas_trabajadas"] == "09:00:00"
        )  # Original hours preserved
        assert (
            result.iloc[0]["horas_esperadas"] == "08:00:00"
        )  # Original hours preserved
    ...
    def test_clasificar_faltas_con_permisos_basic(self):
        ...
        df = pd.DataFrame(
            {"employee": ["EMP001"], "tipo_retardo": ["Falta"], "tiene_permiso": [True]}
        )
    ...
    def test_marcar_dias_no_contratado_lorenzo_case(self):
        ...
        dates = pd.date_range(
            "2025-07-01", "2025-07-15", freq="D"
        ).date  # Only show pre-joining dates
        df = pd.DataFrame(
            {
        ...
        for index, row in result.iterrows():
            assert row["tipo_retardo"] == "No Contratado", (
                f"Day {row['dia']} should be 'No Contratado' but is '{row['tipo_retardo']}'"
            )
...
class TestAttendanceProcessorIntegration:
    ...
    def test_full_processing_pipeline(self):
        ...
        df = processor.ajustar_horas_esperadas_con_permisos(
            df, permisos_dict, cache_horarios
        )
