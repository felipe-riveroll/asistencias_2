...
class TestAPIClient:
    ...
    def test_fetch_checkins_success(self):
        ...
        with patch("config.API_SECRET", "test_secret"), patch(
    ...
    @patch("api_client.get_api_headers")
    @patch("api_client.requests.get")
    def test_fetch_leave_applications_missing_credentials(
        self, mock_get, mock_get_headers
    ):
    ...
    def test_fetch_employee_joining_dates_success(self):
        ...
        with patch("config.API_SECRET", "test_secret"), patch(
            ...
            assert json.loads(call_args[1]["params"]["fields"]) == [
                "employee",
                "date_of_joining",
            ]
    ...
    def test_fetch_employee_joining_dates_pagination(self):
        ...
        with patch("config.API_SECRET", "test_secret"), patch(
            ...
            first_response.json.return_value = {
                "data": [
                    {"employee": f"EMP{i:03d}", "date_of_joining": "2022-01-01"}
                    for i in range(100)
                ]
            }
...
class TestAPIClientIntegration:
    ...
    def test_full_api_workflow(self):
        ...
        with patch.object(client, "fetch_checkins") as mock_checkins, patch.object(
            client, "fetch_leave_applications"
        ) as mock_leaves:
            # Mock realistic data
