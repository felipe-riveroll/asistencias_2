...
class TestResumenPeriodo:
    ...
    def test_resumen_periodo_diferencia_negativa(self):
        ...
        )  # 13 horas trabajadas vs 16 esperadas
    ...
    def test_resumen_periodo_horas_descanso_netas(self):
        """Prueba que el resumen use horas trabajadas netas (después de descanso)."""
        df = pd.DataFrame(
            {
                "employee": ["EMP001", "EMP001", "EMP002", "EMP002"],
                "Nombre": ["Juan Pérez", "Juan Pérez", "María García", "María García"],
                ...
                "horas_descanso": [
                    ...
                    "00:00:00",
                    "00:00:00",
                ],  # 1 hora de descanso el primer día
                "es_retardo_acumulable": [0, 0, 0, 0],
                "es_falta_ajustada": [0, 0, 0, 0],
                "es_falta": [0, 0, 0, 0],
                "falta_justificada": [0, 0, 0, 0],
            }
        )
        ...
        emp1 = resultado[resultado["employee"] == "EMP001"].iloc[0]
        assert emp1["total_horas_trabajadas"] == "15:00:00"  # Netas (7+8)
        assert emp1["total_horas_esperadas"] == "16:00:00"  # Originales (8+8)
        assert emp1["total_horas_descanso"] == "01:00:00"  # 1 hora de descanso
        assert emp1["total_horas"] == "16:00:00"  # Esperadas - Descontadas
        assert emp1["diferencia_HHMMSS"] == "-01:00:00"  # 15 trabajadas - 16 esperadas

        # Verificar EMP002: 8 horas + 8 horas = 16 horas netas
        emp2 = resultado[resultado["employee"] == "EMP002"].iloc[0]
        assert emp2["total_horas_trabajadas"] == "16:00:00"  # Netas (8+8)
        assert emp2["total_horas_esperadas"] == "16:00:00"  # Originales (8+8)
        assert emp2["total_horas_descanso"] == "00:00:00"  # Sin descanso
        assert emp2["total_horas"] == "16:00:00"  # Esperadas - Descontadas
        assert emp2["diferencia_HHMMSS"] == "00:00:00"  # 16 trabajadas - 16 esperadas

    def test_resumen_periodo_verificacion_suma_netas(self):
        """Prueba que la suma de horas trabajadas en detalle coincida con el resumen."""
        df = pd.DataFrame(
            {
                "employee": ["EMP001", "EMP001", "EMP001", "EMP002", "EMP002"],
                "Nombre": [
                ...
                "es_retardo_acumulable": [0, 0, 0, 0, 0],
                "es_falta_ajustada": [0, 0, 0, 0, 0],
                "es_falta": [0, 0, 0, 0, 0],
                "falta_justificada": [0, 0, 0, 0, 0],
            }
        )
        ...
        emp1 = resultado[resultado["employee"] == "EMP001"].iloc[0]
        assert emp1["total_horas_trabajadas"] == "21:30:00"

        # EMP002: 8:00 + 7:30 = 15:30
        emp2 = resultado[resultado["employee"] == "EMP002"].iloc[0]
        assert emp2["total_horas_trabajadas"] == "15:30:00"

        # Verificar que la diferencia sea correcta
        # EMP001: 21:30 trabajadas - 24:00 esperadas = -2:30
        assert emp1["diferencia_HHMMSS"] == "-02:30:00"

        # EMP002: 15:30 trabajadas - 16:00 esperadas = -0:30
        assert emp2["diferencia_HHMMSS"] == "-00:30:00"
