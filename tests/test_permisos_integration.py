...
class TestProcesarPermisosEmpleados:
    ...
    def test_empleado_permiso_un_dia(self):
        ...
        assert (
            result["25"][date(2025, 7, 16)]["leave_type"]
            == "Incapacidad por enfermedad"
        )
        assert (
            result["25"][date(2025, 7, 16)]["employee_name"]
            == "Karla Ivette Chimal Moreno"
        )
    ...
    def test_empleado_permiso_multiples_dias(self):
        ...
        for expected_date in expected_dates:
            ...
            assert (
                result["47"][expected_date]["leave_type"]
                == "Vacaciones (después de 4 años)"
            )
    ...
    def test_multiples_permisos_mismo_periodo(self):
        ...
        assert (
            result["47"][date(2025, 7, 9)]["leave_type"]
            == "Vacaciones (después de 4 años)"
        )
        assert (
            result["47"][date(2025, 7, 10)]["leave_type"]
            == "Permiso sin goce de sueldo"
        )
        assert (
            result["47"][date(2025, 7, 11)]["leave_type"]
            == "Vacaciones (después de 4 años)"
        )
...
class TestAjustarHorasEsperadasConPermisos:
    ...
    def test_permiso_un_dia(self):
        ...
        emp25_dia16 = result[
            (result["employee"] == "25") & (result["dia"] == date(2025, 7, 16))
        ]
        ...
        emp25_dia17 = result[
            (result["employee"] == "25") & (result["dia"] == date(2025, 7, 17))
        ]
    ...
    def test_permiso_multiples_dias(self):
        ...
        emp1_sin_permiso = result[
            (result["employee"] == "1") & (result["dia"] == date(2025, 7, 16))
        ]
...
class TestClasificarFaltasConPermisos:
    ...
    def test_faltas_justificadas_con_permisos(self):
        ...
        emp1_dia17 = result[
            (result["employee"] == "1") & (result["dia"] == date(2025, 7, 17))
        ]
...
class TestIntegracionCompleta:
    ...
    def test_flujo_completo_con_permisos(self):
        ...
        emp1_dia17 = df_final[
            (df_final["employee"] == "1") & (df_final["dia"] == date(2025, 7, 17))
        ]
        assert emp1_dia17.iloc[0]["falta_justificada"] == True
        assert emp1_dia17.iloc[0]["tipo_falta_ajustada"] == "Falta Justificada"
        ...
        emp2_dia16 = df_final[
            (df_final["employee"] == "2") & (df_final["dia"] == date(2025, 7, 16))
        ]
...
class TestCasosEspeciales:
    ...
    def test_dataframe_vacio(self):
        ...
        result_ajuste = ajustar_horas_esperadas_con_permisos(
            df_vacio, permisos_dict, cache_horarios
        )
    ...
    def test_permiso_con_horas_esperadas_cero(self):
        ...
        result = ajustar_horas_esperadas_con_permisos(
            df_test, permisos_dict, cache_horarios
        )
