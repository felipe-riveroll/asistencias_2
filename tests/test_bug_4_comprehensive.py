...
def test_bug_4_comprehensive():
    ...
    df = processor.process_checkins_to_dataframe(
        checkin_data, "2025-07-04", "2025-07-07"
    )
    ...
    assert sabado["checado_1"] == "18:08:41", (
        f"Entrada sábado incorrecta: {sabado['checado_1']}"
    )
    assert sabado["checado_2"] == "02:10:56", (
        f"Salida sábado incorrecta: {sabado['checado_2']}"
    )
    ...
    assert viernes["checado_1"] == "23:15:30", (
        f"Entrada viernes incorrecta: {viernes['checado_1']}"
    )
    assert viernes["checado_2"] == "01:45:15", (
        f"Salida viernes incorrecta: {viernes['checado_2']}"
    )
    ...
    if not juan_sabado.empty:
        ...
        assert (
            pd.isna(sabado_j.get("checado_1")) or sabado_j.get("checado_1") is None
    ...
    assert lunes["checado_1"] == "08:30:00", (
        f"Entrada lunes incorrecta: {lunes['checado_1']}"
    )
    assert lunes["checado_2"] == "17:15:00", (
        f"Salida lunes incorrecta: {lunes['checado_2']}"
    )
