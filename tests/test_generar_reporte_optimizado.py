...
class TestGenerarReporteOptimizado:
    ...
    def test_calcular_proximidad_horario_parametrizado(
        self, checada, hora_prog, esperado
    ):
    ...
    def test_calcular_proximidad_horario_casos_extremos(self):
        ...
        # Casos extremos - verificar que devuelve valores razonables
        resultado1 = calcular_proximidad_horario("23:59:59", "00:00")
        assert resultado1 > 0  # Debe ser un valor positivo

        resultado2 = calcular_proximidad_horario("00:00:01", "23:59")
        assert resultado2 > 0  # Debe ser un valor positivo
    ...
    def test_analizar_asistencia_con_horarios_cache(self):
        ...
        df_analizado = analizar_asistencia_con_horarios_cache(
            df_test, cache_horarios_especifico
        )
    ...
    def test_procesar_horarios_con_medianoche(self, cache_horarios_nocturno):
        ...
        df_procesado = procesar_horarios_con_medianoche(
            df_nocturno, cache_horarios_nocturno
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("08:31:00", "08:00", "Retardo"),
            ("09:00:00", "08:00", "Retardo"),
            ("09:01:00", "08:00", "Falta Injustificada"),
            (None, "08:00", "Falta"),
        ],
    )
    def test_analizar_retardo_casos_especificos_parametrizado(
        self, checada, hora_prog, esperado
    ):
        ...
        # Analizar asistencia
        df_analizado = analizar_asistencia_con_horarios_cache(
            df_caso, cache_horarios_especifico
        )
    ...
    def test_calculo_horas_trabajadas(self):
        ...
        df = process_checkins_to_dataframe(
            datos_multiples_checadas, "2025-01-15", "2025-01-15"
        )
    ...
    def test_manejo_dias_no_laborables(self):
        ...
        # Analizar asistencia
        df_analizado = analizar_asistencia_con_horarios_cache(
            df_no_laborable, cache_horarios_vacio
        )
...
class TestIntegracion:
    ...
    def test_flujo_completo_analisis(
        self, checkin_data_integracion, cache_horarios_integracion
    ):
        ...
        df_procesado = procesar_horarios_con_medianoche(
            df_base, cache_horarios_integracion
        )
        assert not df_procesado.empty
    ...
    def test_generar_resumen_periodo_calculo_corregido(self, mock_print, tmp_path):
        ...
        try:
            ...
            assert (
                emp001["total_horas_trabajadas"] == "18:00:00"
            )  # 10:00 + 8:00 (horas originales trabajadas)
            assert (
                emp001["total_horas_esperadas"] == "18:00:00"
            )  # 9:00 + 9:00 (horas esperadas originales)
            assert (
                emp001["total_horas_descontadas_permiso"] == "09:00:00"
            )  # Solo del día 16
            assert (
                emp001["total_horas"] == "09:00:00"
            )  # 18:00 - 9:00 (esperadas - descontadas)
            assert (
                emp001["diferencia_HHMMSS"] == "+09:00:00"
            )  # 18:00 - 9:00 = +9:00 (horas extra)

            # Verificar cálculos específicos para EMP002 (déficit de horas)
            emp002 = df_resumen[df_resumen["employee"] == "EMP002"].iloc[0]
            assert (
                emp002["total_horas_trabajadas"] == "07:00:00"
            )  # 7:00 + 0:00 (horas originales trabajadas)
            assert (
                emp002["total_horas_esperadas"] == "18:00:00"
            )  # 9:00 + 9:00 (horas esperadas originales)
            assert (
                emp002["total_horas_descontadas_permiso"] == "00:00:00"
            )  # Sin permisos
            assert (
                emp002["total_horas"] == "18:00:00"
            )  # 18:00 - 0:00 (esperadas - descontadas)
            assert (
                emp002["diferencia_HHMMSS"] == "-11:00:00"
            )  # 7:00 - 18:00 = -11:00 (déficit)

            # Verificar cálculos específicos para EMP003 (horas extra sin permisos)
            emp003 = df_resumen[df_resumen["employee"] == "EMP003"].iloc[0]
            assert (
                emp003["total_horas_trabajadas"] == "12:00:00"
            )  # 12:00 (horas originales trabajadas)
            assert (
                emp003["total_horas_esperadas"] == "08:00:00"
            )  # 8:00 (horas esperadas originales)
            assert (
                emp003["total_horas_descontadas_permiso"] == "00:00:00"
            )  # Sin permisos
            assert (
                emp003["total_horas"] == "08:00:00"
            )  # 8:00 - 0:00 (esperadas - descontadas)
            assert (
                emp003["diferencia_HHMMSS"] == "+04:00:00"
            )  # 12:00 - 8:00 = +4:00 (horas extra)
    ...
    def test_generar_resumen_periodo_con_permisos_complejos(self, mock_print, tmp_path):
        ...
        try:
            ...
            assert (
                emp001["total_horas_descontadas_permiso"] == "13:00:00"
            )  # 0:00 + 9:00 + 4:00
            assert emp001["total_horas"] == "13:00:00"  # 26:00 - 13:00
            assert (
                emp001["faltas_justificadas"] == 1
            )  # Solo el día 16 tiene falta justificada
            ...
            assert (
                emp002["total_horas_descontadas_permiso"] == "09:00:00"
            )  # 0:00 + 9:00
            assert emp002["total_horas"] == "09:00:00"  # 18:00 - 9:00
            assert (
                emp002["faltas_justificadas"] == 1
            )  # Solo el día 16 tiene falta justificada
        ...
        df_test = pd.DataFrame(
            {
                ...
                "horas_trabajadas": [
                    "09:00:00",
                    "08:00:00",
                ],  # Total trabajadas: 17:00:00
                "horas_esperadas": [
                    "09:00:00",
                    "00:00:00",
                ],  # Ajustadas por permiso: 9:00:00
                "horas_esperadas_originales": [
                    "09:00:00",
                    "09:00:00",
                ],  # Originales: 18:00:00
