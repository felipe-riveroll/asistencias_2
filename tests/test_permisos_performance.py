...
class TestRendimientoPermisos:
        ...
        for emp_id in range(1, 101):  # 100 empleados
            for day_offset in range(0, 365, 30):  # Un permiso cada 30 días
                ...
                large_leave_data.append({
                    ...
                    "to_date": (current_date + timedelta(days=2)).strftime("%Y-%m-%d"),
                    "status": "Approved"
                })
    ...
    def test_ajustar_dataframe_grande(self):
        ...
        permisos_dict = {
            "1": {dates[0]: {"leave_type": "Vacaciones", "employee_name": "Employee 1"}},
            "50": {dates[50]: {"leave_type": "Incapacidad", "employee_name": "Employee 50"}},
            "100": {dates[99]: {"leave_type": "Permiso", "employee_name": "Employee 100"}}
        }
        
        cache_horarios = {str(i): {1: None, 2: None, 3: None, 4: None, 5: None, 6: None, 7: None} 
                         for i in range(1, 101)}
        
        # Medir tiempo
        start_time = time.time()
        result = ajustar_horas_esperadas_con_permisos(large_df, permisos_dict, cache_horarios)
        processing_time = time.time() - start_time
...
class TestCasosExtremos:
    ...
    def test_dataframe_solo_una_fila(self):
        ...
        # Ajustar horas
        result_ajuste = ajustar_horas_esperadas_con_permisos(single_row_df.copy(), permisos_dict, cache_horarios)
...
class TestIntegracionRobustez:
    ...
    def test_flujo_completo_con_datos_faltantes(self):
        ...
        df_test = pd.DataFrame({
            'employee': ['1', '1', '2', '2'],
            'dia': [date(2025, 7, 16), date(2025, 7, 17), date(2025, 7, 16), date(2025, 7, 17)],
        ...
        # Ajustar horas
        df_ajustado = ajustar_horas_esperadas_con_permisos(df_test.copy(), permisos_dict, cache_horarios)
        ...
        emp1_dia16 = df_final[(df_final['employee'] == '1') & (df_final['dia'] == date(2025, 7, 16))]
        emp2_dia17 = df_final[(df_final['employee'] == '2') & (df_final['dia'] == date(2025, 7, 17))]
