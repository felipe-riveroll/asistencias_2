...
def crear_datos_prueba():
    ...
    def calcular_tipo_retardo(hora_programada, hora_checada):
        ...
        try:
            ...
            dt_programada = datetime.combine(
                datetime.strptime(fecha_base, "%Y-%m-%d").date(),
                datetime.strptime(hora_programada, "%H:%M:%S").time(),
            )
            dt_checada = datetime.combine(
                datetime.strptime(fecha_base, "%Y-%m-%d").date(),
    ...
    for caso in casos_prueba:
        ...
        if horario:
            ...
            caso["tipo_retardo"] = calcular_tipo_retardo(
                horario["hora_entrada"], caso["checado_1"]
            )
        else:
...
class TestColoreadoChecadas:
    ...
    def setup_method(self):
        ...
        self.mock_cell = type(
            "MockCell",
            (),
            {"fill": None, "font": type("MockFont", (), {"color": None})()},
        )()
    ...
    def test_entrada_a_tiempo_sin_color(self):
        ...
        caso_a_tiempo = next(
            c for c in self.casos_prueba if c["descripcion"] == "A Tiempo"
        )
    ...
    def test_retardo_amarillo(self):
        ...
        caso_retardo = next(
            c for c in self.casos_prueba if c["descripcion"] == "Retardo"
        )
    ...
    def test_falta_injustificada_rojo(self):
        ...
        caso_falta = next(
            c for c in self.casos_prueba if c["descripcion"] == "Falta Injustificada"
        )
    ...
    def test_horarios_dinamicos_diferentes_empleados(self):
        ...
        for caso in casos_retardo:
            ...
            assert self.mock_cell.fill == self.fill_amarillo, (
                f"Falló para empleado {caso['employee']} con horario {caso['hora_entrada_programada']}"
            )
    ...
    def test_solo_retardo_amarillo_falta_injustificada_rojo(self):
        ...
        for caso in casos_retardo:
            ...
            assert self.mock_cell.fill == self.fill_amarillo, (
                f"Retardo debe ser amarillo para {caso['employee']}"
            )
        ...
        for caso in casos_falta:
            ...
            assert self.mock_cell.fill == self.fill_rojo, (
                f"Falta Injustificada debe ser rojo para {caso['employee']}"
            )
            assert self.mock_cell.font.color is not None, (
                f"Falta Injustificada debe tener texto blanco para {caso['employee']}"
            )
        ...
        for caso in casos_a_tiempo:
            ...
            assert self.mock_cell.fill is None, (
                f"A Tiempo no debe tener color para {caso['employee']}"
            )
