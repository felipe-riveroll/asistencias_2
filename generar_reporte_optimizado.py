...
<body>
    ...
    <div class="container">
        ...
        <div id="dashboard" class="tab-content active">
            ...
            <div class="chart-container">
                ...
                <div class="chart-subtitle">Distribución de tipos de ausencias y su impacto operativo</div>
    ...
    <script>
        ...
        // Gráfica de impacto por ausencias
        function createAbsenceImpactChart() {{
            console.log("Creando gráfica de impacto por ausencias...");
            ...
            // Calcular datos de impacto mejorados
            const totalUnjustified = employeeData.reduce((sum, emp) => sum + (emp.totalAbsences || 0), 0);
            const totalJustified = employeeData.reduce((sum, emp) => sum + (emp.justifiedAbsences || 0), 0);
            const totalDelays = employeeData.reduce((sum, emp) => sum + (emp.delays || 0), 0);
            
            // Calcular porcentajes de impacto
            const totalEmployees = employeeData.length;
            const totalPossibleDays = totalEmployees * diasLaborales;
            ...
            const impactData = [
                {{ 
                    ...
                    impact: "Alto",
                    color: "#dc3545",
                    percentage: ((totalUnjustified / totalPossibleDays) * 100).toFixed(1)
                }},
                {{ 
                    type: "Faltas\\nJustificadas", 
                    ...
                    impact: "Medio",
                    color: "#ffc107",
                    percentage: ((totalJustified / totalPossibleDays) * 100).toFixed(1)
                }},
                {{ 
                    type: "Retardos", 
                    count: totalDelays, 
            ...
            console.log("Datos de impacto:", impactData);
            const maxCount = d3.max(impactData, d => d.count) || 1;
            ...
            svg.selectAll(".bar")
                ...
                .attr("stroke-width", 2)
                .on("mouseover", (event, d) => {{
                    const tooltipContent = d.percentage !== "N/A" 
                        ? `<strong>${{d.type.replace('\\n', ' ')}}</strong><br/>
                           Cantidad: ${{d.count}}<br/>
                           Impacto: ${{d.impact}}<br/>
                           % Capacidad: ${{d.percentage}}%`
            ...
            // Etiquetas de valores con porcentajes
            svg.selectAll(".value-label")
                .data(impactData)
                .enter().append("text")
                .attr("class", "value-label")
                .attr("x", d => xBandScale(d.type) + xBandScale.bandwidth() / 2)
                .attr("y", d => yLinearScale(d.count) - 8)
            ...
            // Etiquetas de porcentaje para faltas
            svg.selectAll(".percentage-label")
                .data(impactData.filter(d => d.percentage !== "N/A"))
                .enter().append("text")
                .attr("class", "percentage-label")
                .attr("x", d => xBandScale(d.type) + xBandScale.bandwidth() / 2)
                .attr("y", d => yLinearScale(d.count) - 25)
                .attr("text-anchor", "middle")
                .style("font-size", "11px")
                .style("fill", "#666")
                .text(d => `(${{d.percentage}}%)`);

            console.log("Gráfica de impacto completada");
        }}
