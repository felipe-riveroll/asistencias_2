...
class TestAttendanceReportManager:
    ...
    def test_generate_attendance_report_success(
        self,
        mock_determine_period,
        mock_mapear_horarios,
        mock_obtener_horarios,
        mock_procesar_permisos,
        mock_obtener_codigos,
        mock_connect_db,
        mock_validate_api,
    ):
        ...
        mock_obtener_horarios.return_value = {
            "primera": [
                {"employee": "EMP001", "hora_entrada": "08:00", "hora_salida": "17:00"}
            ]
        }
        mock_mapear_horarios.return_value = {"EMP001": {"08:00": "17:00"}}

        # Mock API client methods
        self.manager.api_client.fetch_checkins = Mock(
            return_value=[
                {
                    "employee": "EMP001",
        ...
        self.manager.processor.process_checkins_to_dataframe = Mock(
            return_value=mock_df
        )
        self.manager.processor.procesar_horarios_con_medianoche = Mock(
            return_value=mock_df
        )
        self.manager.processor.analizar_asistencia_con_horarios_cache = Mock(
            return_value=mock_df
        )
        self.manager.processor.aplicar_calculo_horas_descanso = Mock(
            return_value=mock_df
        )
        self.manager.processor.ajustar_horas_esperadas_con_permisos = Mock(
            return_value=mock_df
        )
        self.manager.processor.aplicar_regla_perdon_retardos = Mock(
            return_value=mock_df
        )
        self.manager.processor.clasificar_faltas_con_permisos = Mock(
            return_value=mock_df
        )
        ...
        self.manager.report_generator.save_detailed_report = Mock(
            return_value="detailed_report.csv"
        )
        self.manager.report_generator.generar_resumen_periodo = Mock(
            return_value=mock_resumen
        )
        self.manager.report_generator.generar_reporte_html = Mock(
            return_value="dashboard.html"
        )
        self.manager.report_generator.generar_reporte_excel = Mock(
            return_value="report.xlsx"
        )
    ...
    def test_generate_attendance_report_no_schedules(self, mock_obtener_horarios):
        ...
        # Setup basic mocks
        with patch("main.validate_api_credentials"), patch(
            "main.connect_db", return_value=MagicMock()
        ), patch("main.obtener_codigos_empleados_api", return_value=["EMP001"]), patch(
            "main.determine_period_type", return_value=(True, False)
...
class TestEndToEndIntegration:
    ...
    def test_full_pipeline_integration(self, mock_connect_db, mock_validate_api):
        ...
        with patch.object(
            manager.api_client, "fetch_checkins", return_value=checkin_data
        ), patch.object(
            manager.api_client, "fetch_leave_applications", return_value=[]
        ), patch("main.obtener_codigos_empleados_api", return_value=["EMP001"]), patch(
            "main.procesar_permisos_empleados", return_value={}
        ), patch("main.determine_period_type", return_value=(True, False)), patch(
            "main.obtener_horarios_multi_quincena",
            return_value={"primera": [{"employee": "EMP001"}]},
        ), patch(
            "main.mapear_horarios_por_empleado_multi",
            return_value={
                "EMP001": {
                    True: {
                        1: {
