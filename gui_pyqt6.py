...
from utils import determine_period_type, obtener_codigos_empleados_api
    ...
    def generate_attendance_report(
        ...
        try:
            ...
            if not checkin_records:
                return {
                    "success": False,
                    "error": f"No se encontraron registros de entrada/salida para el dispositivo '{device_filter}' en el período {start_date} al {end_date}. Posibles causas: 1) No hay registros para este dispositivo en el período, 2) El filtro de dispositivo no coincide con ningún dispositivo, 3) Los dispositivos de esta sucursal tienen nombres diferentes.",
                }
            ...
            if not any(horarios_por_quincena.values()):
                ...
                return {
                    "success": False,
                    "error": f"No hay horarios para la sucursal '{sucursal}'. Verifica que los empleados tengan horarios asignados en la base de datos.",
                }
            ...
            self.emit_progress(
                5, f"💾 Paso 5/7: Reportes CSV generados en {step5_time:.1f}s"
            )
            ...
            if not df_resumen.empty:
                ...
                self.emit_progress(
                    6, f"🌐 Paso 6/7: Dashboard HTML generado en {step6_time:.1f}s"
                )
            else:
            ...
            if not df_resumen.empty:
                ...
                self.emit_progress(
                    7, f"📊 Paso 7/7: Reporte Excel generado en {step7_time:.1f}s"
                )
            else:
...
class ReportWorkerSignals(QObject):
    ...
    )  # message, current_step, total_steps, elapsed_time, records_processed
    ...
    def setup_ui(self):
        ...
        if self.result.get("success"):
            ...
            details_layout.addWidget(
                QLabel(f"👥 Empleados procesados: {employees_processed}")
            )
    ...
    def open_excel_file(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Error", f"No se pudo abrir el archivo Excel:\n{str(e)}"
            )
...
class AttendanceReportGUI(QMainWindow):
    ...
    BRANCH_OPTIONS = [
        ...
        ("NAVE", "Nave", "%nave%"),
        ("RIO BLANCO", "Rio Blanco", "%Rio%"),  # Filtro principal para Rio Blanco
    ]
    ...
    def validate_dates(self):
        ...
        if end_date <= start_date:
            self.date_validation_label.setText(
                "⚠️ La fecha final debe ser posterior a la fecha de inicio"
            )
    ...
    def generate_report(self):
        ...
        self.update_status(
            f"Iniciando reporte para {sucursal} ({start_date} - {end_date})"
        )
    ...
    def on_report_finished(self, result: dict):
        ...
        self.progress_bar.setValue(
            100 if result.get("success") else self.progress_bar.value()
        )
        self.progress_bar.setVisible(False)
