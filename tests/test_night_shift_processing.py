...
class TestNightShiftProcessing(unittest.TestCase):
    ...
    def test_process_checkins_to_dataframe(self):
        ...
        checadas_03 = [
            df_03.iloc[0][f"checado_{i}"]
            for i in range(1, 4)
            if f"checado_{i}" in df_03.columns
            and pd.notna(df_03.iloc[0][f"checado_{i}"])
        ]
        checadas_04 = [
            df_04.iloc[0][f"checado_{i}"]
            for i in range(1, 4)
            if f"checado_{i}" in df_04.columns
            and pd.notna(df_04.iloc[0][f"checado_{i}"])
        ]
    ...
    def test_map_shift_date_function(self):
        ...
        def map_shift_date(
            ...
            try:
                ...
                if checada_time_obj <= salida_time or (
                    checada_time_obj.hour == salida_time.hour
                    and checada_time_obj.minute <= salida_time.minute + 5
                ):
    ...
    def test_night_shift_processing_integration(self):
        ...
        columnas_existentes_proc = [
            col for col in columnas_proc if col in df_procesado.columns
        ]
